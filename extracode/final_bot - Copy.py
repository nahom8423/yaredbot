...
def to_ethiopian_date_amharic(gregorian_date):
    day, month, year = to_ethiopian_date(gregorian_date)
    amharic_day = arabic_to_geez(day)
    amharic_month = ethiopian_months[month - 1]
    amharic_year = arabic_to_geez(year)
    return f"{amharic_day} {amharic_month} {amharic_year}"

async def update_channel_name():
    voice_channel_name = to_ethiopian_date_amharic(datetime.now())
    guild = discord.utils.get(bot.guilds, id=GUILD_ID)
    voice_channel = discord.utils.get(guild.voice_channels, id=YOUR_VOICE_CHANNEL_ID)  # Replace with your voice channel's ID
    await voice_channel.edit(name=voice_channel_name)

from discord.ext import tasks

@tasks.loop(hours=24)
async def update_channel_name_daily():
    await update_channel_name()

@bot.event
async def on_ready():
    print(f'Logged in as {bot.user.name}')
    update_channel_name_daily.start()
